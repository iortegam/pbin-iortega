...
class WebParser(FTIRdataClient):
    ...
    def __init__(self,WebPageParse_IP="192.168.1.102",TCP_IP_in="192.168.1.100",TCP_Port_in=5555,BufferSize_in=4024):
...
class FTIRwebParser(WebParser):
    ...
    def __init__(self,WebPageParse_IP="192.168.1.102",TCP_IP_in="192.168.1.100",TCP_Port_in=5555,BufferSize_in=1024):
...
class APCwebParser(WebParser):
    
    def __init__(self,WebPageParse_IP="192.168.1.104",TCP_IP_in="192.168.1.100",TCP_Port_in=5555,BufferSize_in=4024):
    ...
    def getInputVoltage(self):
        ...
        # Extract Table
        #--------------
        table = soup.find("span",{"id":"langVAC"})
        
        #------------------
        # Get Input voltage
        #------------------
        self.inVolt = float(table.previous.strip())
        
        if self.inVolt == 0.0: self.data["UPS_POWERFAIL"] = "True"
        else:                  self.data["UPS_POWERFAIL"] = "False"
...
if __name__ == "__main__":
    ...
    try:
        ...
        ctlFile  = "/data/iortega/pbin/remote/TAB_Defaults.input" 
    ...
    while True:
        ...
        #--------------------------------------
        # Get Input Power Voltage from UPS Page
        #--------------------------------------
        try:
            apcData.getInputVoltage()
        except:
            print "Unable to get input voltage from UPS status page!"
            
        #---------------------------------------
        # Send Power failure flag to data server
        #---------------------------------------
        try:
            apcData.sendData()
        except:
            print "Unable to send power failure flag to data server"
