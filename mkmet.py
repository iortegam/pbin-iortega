...
def main():
    ...
    for indvYear in years:
        ...
        #------------------------
        houseFileNew = outDataDir + statstr.upper() + '_MET_all_' + str(indvYear) + '.dat'
        ...
        for indvDay in daysYear:
            ...
            if (statstr.lower() == 'mlo'):
                ...
                if indvDay >= dt.date(2005,06,13) and indvDay < dt.date(2005,10,14):
                    srchstr = 'HOUSE.LOG'
                elif indvDay >= dt.date(2013,07,19) and indvDay < dt.date(2013,10,25):
                    srchstr = 'House.log'
                else:
                    srchstr = 'house.log'
        ...
        with open(houseFileNew, wmode) as fopen:
            ...
            if (wmode == 'wb') and (statstr.lower() == 'mlo'):
                ...
                # Print header
                #-------------
                fopen.write('Date [YYYYMMDD]    Time [HH:MM:SS] Temperature [C] Wind Dir [West of South]    Wind Speed [ms^-1]  Pressure [mbar] RH[%]\n')
                # fopen.write('#   LABEL                       Units          Column    Missing Value\n')
                # fopen.write('#---------------------------------------------------------------------\n')
                # fopen.write('#   Date                        YYYYMMDD       1         NA           \n')
                # fopen.write('#   Time                        HH:MM:SS       2         NA           \n')
            ...
            if (statstr.lower() == 'mlo'):
                ...
                order = {'Date':0,'Time':1, 'Outside_T':2, 'WindDir_W_of_S':3, 'Wind_Speed_mps':4, 'Atm_Press':5, 'Outside_RH':6} 
