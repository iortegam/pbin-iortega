     ...
     #-------------------------
        ...
        if ngases >= 2:
            ...
            if k <= ((ngases/2) - 1):
                ...
                if k == (ngases/2)-1: ax[k, 0].set_xlabel('Averaging Kernels')
                ax[k, 0].grid(True)        
                #ax[k, 0].set_title('Averaging Kernels Scale Factor')
                ax[k, 0].set_ylim(0, 50)
                ax[k, 0].annotate(gasname, xy=(.95, .89), xycoords='axes fraction', fontsize=24,
                ...
                ax[k-(ngases/2), 1].set_ylabel('Altitude [km]')
                if k == ngases-1: ax[k-(ngases/2), 1].set_xlabel('Averaging Kernels')
                ax[k-(ngases/2), 1].grid(True)        
                #ax[k-(ngases/2), 1].set_title('Averaging Kernels Scale Factor')
                ax[k-(ngases/2), 1].set_ylim(0, 50)
                ax[k-(ngases/2), 1].annotate(gasname, xy=(.95, .89), xycoords='axes fraction', fontsize=24,
                    horizontalalignment='right', verticalalignment='center')
